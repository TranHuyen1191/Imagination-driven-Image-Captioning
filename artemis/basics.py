...
from artemis.language_preprocessing import unquote_words, expand_contractions
from artemis.language_preprocessing import manual_sentence_spelling, manual_tokenized_sentence_spelling
from artemis.spelling import sentence_spelling_dictionary as artemis_sentence_spelling_dictionary
from artemis.spelling import token_spelling_dictionary as artemis_token_spelling_dictionary
from artemis.spelling import missing_from_glove_but_are_actual_words
from artemis.word_embeddings import load_glove_pretrained_embedding
...
def tokenize_and_spell_modified(df, golden_vocabulary, speller,missed_tokens, tokenizer, parallel=True, inplace=True, spell_check=True):
    def automatic_token_speller(token_list, max_edit_distance=1):
        new_tokens = []
        for token in token_list:
            if token in golden_vocabulary:
