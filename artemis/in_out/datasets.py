...
class AffectiveCaptionDataset(Dataset):
    ...
    """
    def __init__(self, image_files, tokens, emotions, n_emotions=8, img_transform=None, one_hot_emo=True):
...
class ImageClassificationDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        res = {'image': img, 'label': label, 'index': index, 'img_file': self.image_files[index]}
        return res
...
import pdb
def default_grounding_dataset_from_affective_loader_predEmo(loader, img2emo_clf=None, device=None, n_workers=None):
    """
    Convenience function. Given a loader carrying an affective dataset, make a new loader only w.r.t.
    unique images of the dataset, & optionally add to each image the emotion predicted by the img2emo_clf.
    The new loader can be used to sample utterances over the unique images.
    ...
    #pdb.set_trace()
    pred_emotions = np.array([ emo[-1][0] for emo in emotions['emotion']])
    print(pred_emotions[0])

    new_dataset = AffectiveCaptionDataset(img_files, tokens=dummy, emotions=pred_emotions,img_transform=img_trans)
