...
def image_trans(img_dim, lanczos=True):
    """simple transformation/pre-processing of image data."""
...
def df_to_ArtEmisdataset(df, args):
    if args.num_workers == -1:
        n_workers = max_io_workers()
    else:
        n_workers = args.num_workers
    ...
    img_transforms = None
    if load_imgs:
        img_transforms = image_trans(args.img_dim, lanczos=args.lanczos)
    print(img_transforms)
    if args.dataset == 'artemis':
        datasets = pass_artemis_splits_to_datasets(df, load_imgs, img_transforms, args.img_dir, n_emotions=0, one_hot_emo=one_hot_emo)
    elif args.dataset == 'ola': # Objective Language for Art.
        datasets = pass_artemis_splits_to_datasets(df, load_imgs, img_transforms, args.img_dir, n_emotions=0)
    elif args.dataset == 'coco':
        datasets = pass_coco_splits_to_datasets(df, load_imgs, img_transforms)
...
def pass_artemis_splits_to_datasets(df, load_imgs, img_transforms, top_img_dir, n_emotions=8, one_hot_emo=True):
    datasets = dict()
    for split, g in df.groupby('split'):
        g.reset_index(inplace=True, drop=True) # so that direct ([]) indexing in get_item works
        img_files = None
...
def image_emotion_distribution_df_to_pytorch_evaldataset(df, args, drop_thres=None):
    """ Convert the pandas dataframe that carries information about images and emotion (distributions) to a
    dataset that is amenable to deep-learning (e.g., for an image2emotion classifier).
    :param df:
    :param args:
    ...
    img_files = df.apply(lambda x : osp.join(args.img_dir, x.art_style,  x.painting + '.jpg'), axis=1)
    img_files.name = 'image_files'

    dataset = ImageClassificationDataset(img_files, df.emotion_distribution,
                                             img_transform=img_transforms['train'])

    datasets = dataset
    b_size = 1
    dataloaders = torch.utils.data.DataLoader(dataset=dataset,
                                            batch_size=b_size,
                                            shuffle=False,
                                            num_workers=n_workers)
    return dataloaders, datasets
...
def load_saved_speaker_predEmo(args_file, model_ckp, with_data=False, override_args=None, verbose=False):
    """
    :param args_file: saved argparse arguments with model's description (and location of used data)
    :param model_ckp: saved checkpoint with model's parameters.
    :param with_data:
    ...
    if with_data:
        df = read_preprocessed_data_df(args, verbose=True)
        print(df.emotion_label[0])
        df.pred_emotion_label = df.pred_emotion_label.apply(literal_eval)
        df.emotion_label = df.pred_emotion_label
        print(df.emotion_label[0])
        data_loaders, _ = df_to_pytorch_dataset(df, args)
    else:
        data_loaders = None
